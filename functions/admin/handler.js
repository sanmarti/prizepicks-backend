...
const { listPrivateLeagues, getPrivateLeague, updatePrivateLeague, adminCreatePeriod, adminTogglePayment, adminUpdatePeriodPrizes, generateLeagueCalendar, publishLeague, startLeagueWeek, settleLeagueWeek } = require('./private_leagues')
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (routeKey === "POST /admin/gameweeks/{id}/early-settle")  return await earlySettleGameweek(event)
    ...
    if (routeKey === "POST /admin/competitions/{id}/refresh-calendar") return await refreshCompetitionCalendar(event)
    ...
    if (routeKey === "POST /admin/private-leagues/{id}/generate-calendar")           return await generateLeagueCalendar(event)
    if (routeKey === "POST /admin/private-leagues/{id}/publish")                     return await publishLeague(event)
    if (routeKey === "POST /admin/private-leagues/{id}/start-week")                  return await startLeagueWeek(event)
    if (routeKey === "POST /admin/private-leagues/{id}/settle-week")                 return await settleLeagueWeek(event)
